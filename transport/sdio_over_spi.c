 ...
 */
static int sdio_over_spi_error(struct morsectrl_transport *transport,
                     int error_code, char *error_msg)
{
    if (transport->error_function)
        return transport->error_function("SPI", error_code, error_msg);
    return 0;
}
...
static int sdio_over_spi_cmd0(struct morsectrl_transport *transport)
{
    ...
    struct morsectrl_transport_buff *resp_buff =
        transport->tops->read_alloc(transport, cmd_buff->data_len);
    int ret = ETRANSSUCC;

    if (!cmd_buff || !resp_buff)
    {
        ret = -ETRANSERR;
        goto exit;
    }
...
int sdio_over_spi_post_hard_reset(struct morsectrl_transport *transport)
{
    ...
    if (ret)
    {
        transport->error_function("Pre Firmware DL", -ETRANSERR, "Failed to read chip id reg\n");
        return ret;
    }
    ...
    for (ii = 0; ii < 3; ii++)
    {
        ...
        if (ret)
        {
            transport->error_function("Pre Firmware DL", -ETRANSERR,
                                      "Failed to write reset reg\n");
