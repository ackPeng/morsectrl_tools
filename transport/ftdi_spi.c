 ...
 */
static int ftdi_spi_error(struct morsectrl_transport *transport,
                                   int error_code, char *error_msg)
{
    if (transport->error_function)
        return transport->error_function("FTDI_SPI", error_code, error_msg);
    return 0;
}
...
static int ftdi_spi_parse(struct morsectrl_transport *transport,
                          ...
                          const char *cfg_opts)
{
    struct morsectrl_ftdi_spi_cfg *config = &transport->config.ftdi_spi;
    ChannelConfig *chan_config = &config->channel;
    char *cpy;
    char *ptr;
...
{
    struct morsectrl_ftdi_spi_state *state = &transport->state.ftdi_spi;
    struct morsectrl_ftdi_spi_cfg *config = &transport->config.ftdi_spi;
    struct morsectrl_ftdi_spi_chan_info spi_chan_info = {
    ...
    {
        ftdi_spi_error(transport, num_chan, "Too many SPI channels");
        return -ETRANSFTDISPIERR;
    }
    ...
    if (status != FT_OK)
    {
        ftdi_spi_error(transport, status, "Failed to open MPSSE SPI channel");
        return -ETRANSFTDISPIERR;
    }
    ...
    if (status != FT_OK)
    {
        ftdi_spi_error(transport, status, "Failed to init MPSSE SPI channel");
        return -ETRANSFTDISPIERR;
    }
    ...
    if (status != FT_OK)
    {
        ftdi_spi_error(transport, status, "Failed to open MPSSE reset channel");
        return -ETRANSFTDISPIERR;
    }
    ...
    if (status != FT_OK)
    {
        ftdi_spi_error(transport, status, "Failed to init MPSSE reset channel");
        return -ETRANSFTDISPIERR;
    }
                                                       ...
                                                       size_t size)
{
    ...
    if (!transport || (transport->type != MORSECTRL_TRANSPORT_FTDI_SPI))
        return NULL;
...
{
    FT_HANDLE handle = transport->state.ftdi_spi.handle;
    FT_STATUS status;
    ...
    if (status)
    {
        ftdi_spi_error(transport, status, "Failed to set CS");
        return -ETRANSFTDISPIERR;
    }
                             ...
                             bool finish)
{
    ...
    if (read == NULL)
    {
        ...
        ftdi_spi_error(transport, -ETRANSFTDISPIERR, "Empty SPI read");
        return -ETRANSFTDISPIERR;
    }
    ...
    status = SPI_Read(transport->state.ftdi_spi.handle,
                      read->data, read->data_len,
                      &size_transferred, options);

    if (status)
    {
        ftdi_spi_error(transport, status, "Failed to raw read");
        return -ETRANSFTDISPIERR;
    }

    if (size_transferred != read->data_len)
    {
        ftdi_spi_error(transport, status, "Raw read size mismatch");
        return -ETRANSFTDISPIERR;
    }
                       ...
                       bool finish)
{
    ...
    status = SPI_Write(transport->state.ftdi_spi.handle,
                       write->data, write->data_len,
                       &size_transferred, options);

    if (status)
    {
        ftdi_spi_error(transport, status, "Failed to raw write");
        return -ETRANSFTDISPIERR;
    }

    if (size_transferred != write->data_len)
    {
        ftdi_spi_error(transport, status, "Raw write size mismatch");
        return -ETRANSFTDISPIERR;
    }
                            ...
                            bool finish)
{
    ...
    if (!read || !write)
    {
        ftdi_spi_error(transport, 0, "Raw read/write missing buffer");
        return -ETRANSFTDISPIERR;
    }
    ...
    status = SPI_ReadWrite(transport->state.ftdi_spi.handle,
                           read->data, write->data, transfer_size,
                           &size_transferred, options);

    if (status)
    {
        ftdi_spi_error(transport, status, "Failed to raw read/write");
        return -ETRANSFTDISPIERR;
    }

    if (size_transferred != transfer_size)
    {
        ftdi_spi_error(transport, status, "Raw read/write size mismatch");
        return -ETRANSFTDISPIERR;
    }
                         ...
                         struct morsectrl_transport_buff *resp)
{
    ...
    if (!cmd_addr)
    {
        ftdi_spi_error(transport, cmd_addr,
                       "This transport is not supported for production firmware");
        return -ETRANSFTDISPIERR;
    }
    ...
    resp->data_len = response->hdr.len - sizeof(response->status);

    return ETRANSSUCC;

fail:
    ftdi_spi_error(transport, ret, "Failed to send command");
    return ret;
}
...
static int ftdi_spi_reset(struct morsectrl_transport *transport)
{
    ...
    if (status != FT_OK)
    {
        ftdi_spi_error(transport, status, "Failed to get channel config during GPIO reset");
        return -ETRANSFTDISPIERR;
    }
    ...
    if (status != FT_OK)
    {
        ftdi_spi_error(transport, status, "Failed to write GPIO reset line");
        return -ETRANSFTDISPIERR;
    }
    ...
    if (status != FT_OK)
    {
        ftdi_spi_error(transport, status, "Failed to write GPIO JTAG reset line");
        return -ETRANSFTDISPIERR;
    }
...
const struct morsectrl_transport_ops ftdi_spi_ops = {
